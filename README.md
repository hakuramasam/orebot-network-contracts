# OREBOT Agent Gateway

This library provides convenient access to the OREBOT Agent Gateway REST API from TypeScript or JavaScript.
...
| `baseURL` | `string \| null` | `process.env["OREBOT_PROTOCOL_BASE_URL"]` | Override the default API base URL. Pass `null` when selecting a configured environment. |
...
| `logLevel` | `"off" \| "error" \| "warn" \| "info" \| "debug" \| null` | `process.env["OREBOT_PROTOCOL_LOG"]` | Controls request and retry debug logging. |
