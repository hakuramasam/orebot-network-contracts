...
import { Chat, type PromptRequest, type ChatAgentParams } from './resources/chat';
import { Reason, type ReasonAgentParams } from './resources/reason';
import { Research, type ResearchAgentParams } from './resources/research';
import { Code, type CodeAgentCodingParams } from './resources/code';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env["OREBOT_PROTOCOL_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["OREBOT_PROTOCOL_LOG"] or 'warn' if it isn't set.
   */
...
export type OrebotAgentGatewayOptions = ClientOptions;

/**
 * API Client for interfacing with the OrebotAgentGateway API.
 */
export class OrebotAgentGateway {
   ...
   * @param {string} [opts.baseURL=process.env["OREBOT_PROTOCOL_BASE_URL"] ?? https://api.orebot.network] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('OREBOT_PROTOCOL_BASE_URL'),
    apiKeyAuth = readEnv('API_KEY_AUTH'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKeyAuth === undefined) {
      throw new Errors.OrebotAgentGatewayError(
        "The API_KEY_AUTH environment variable is missing or empty; either provide it, or instantiate the OrebotAgentGateway client with an apiKeyAuth option, like new OrebotAgentGateway({ apiKeyAuth: 'My API Key Auth' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKeyAuth,
      ...opts,
      baseURL: baseURL || 'https://api.orebot.network',
    };
    const baseURLOverridden = baseURL !== null && baseURL !== undefined && baseURL !== '';
    const defaultBaseURL = 'https://api.orebot.network';
    this.baseURL = options.baseURL || defaultBaseURL;
    this.timeout = options.timeout ?? OrebotAgentGateway.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('OREBOT_PROTOCOL_LOG'), 'process.env["OREBOT_PROTOCOL_LOG"]', this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('OREBOT_PROTOCOL_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  private async resolveAuthOption(
    ...
    if (!token)
      throw new Errors.OrebotAgentGatewayError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }
  ...
  static OrebotAgentGateway = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static OrebotAgentGatewayError = Errors.OrebotAgentGatewayError;
...
OrebotAgentGateway.Health = Health;
OrebotAgentGateway.Services = Services;
OrebotAgentGateway.Balance = Balance;
OrebotAgentGateway.Chat = Chat;
OrebotAgentGateway.Reason = Reason;
OrebotAgentGateway.Research = Research;
OrebotAgentGateway.Code = Code;
OrebotAgentGateway.Credits = Credits;

export declare namespace OrebotAgentGateway {
  ...
  export { Chat as Chat, type PromptRequest as PromptRequest, type ChatAgentParams as ChatAgentParams };

  export { Reason as Reason, type ReasonAgentParams as ReasonAgentParams };

  export { Research as Research, type ResearchAgentParams as ResearchAgentParams };

  export { Code as Code, type CodeAgentCodingParams as CodeAgentCodingParams };
